# S2-01 Group B Visual Retry 20 Assessment
...
- Playwright and Chrome DevTools navigation return `user cancelled MCP tool call`.
...
- Headless Google Chrome and Brave both exit `-1` for the static `file://` viewer path and create no screenshot.
- URL CLI conversion with `--refresh` fails DNS resolution for all Desmos endpoints.
